...
```{r}
cross_betweenness_all_monthly <- function(affils_by_date, on_cols, start, end, output_file) {
  
  # all_starts <- seq.Date(from = as.Date("1945-01-01"), to = as.Date("1989-01-01"), by = "day")
  # all_ends <- all_starts

  all_starts <- seq.Date(from = start, to = end, by = "month")
  all_ends <- all_starts %m+% months(1) - days(1)

  # all_starts <- seq.Date(from = start, to = end, by = "year")
  # all_ends <- all_starts %m+% years(1) - days(1) 

  # Check if output file already exists -- if it does, resume
  # if (file.exists(output_file)) {
  #   existing_data <- read.csv(output_file)
  #   processed_dates <- unique(existing_data$Start.Date)
  # } else {
  #   processed_dates <- character()
  # }
  ...
  for (i in seq_along(all_starts)) {
    ...
    # skip if date range has already been seen
    # if (as.character(s) %in% processed_dates) next
    ...
    # Append
    write.table(temp, file = output_file, sep = ",", row.names = FALSE, col.names = !file.exists(output_file), append = TRUE)

    # print(paste("Processed:", s, "to", e))
    message(paste("Processed:", s, "to", e))
  }
}
```
...
cross_betweenness_all_monthly(
  affils_by_date = affiliation_dates,
  on_cols = c("Org.ID", "Umbrella", "Subgroup"),
  start = as.Date("1945-01-01"),
  end = as.Date("1989-01-01"),
  output_file = "cb_monthly_results.csv"
)
```

```{r}

all_starts<-seq.Date(from = as.Date("1945-01-01"), to = as.Date("1989-01-01"), by = "month")
all_ends<-all_starts %m+% months(1) - days(1)
...
```{r}
cross_betweenness_norm_all_monthly <- function(affils_by_date, on_cols, start, end, output_file) {
  all_starts <- seq.Date(from = start, to = end, by = "month")
  all_ends <- all_starts %m+% months(1) - days(1)
...
```
```{r}
cross_betweenness_norm_all_monthly(
  affils_by_date = affiliation_dates,
  on_cols = c("Org.ID", "Umbrella", "Subgroup"),
  start = as.Date("1945-01-01"),
  end = as.Date("1989-01-01"),
  output_file = "cb_norm_monthly_results.csv"
)
```
